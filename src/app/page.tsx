...
const getHijriDate = (date: Date): string => { try { const options: Intl.DateTimeFormatOptions = { calendar: 'islamic-umalqura', day: 'numeric', month: 'long', year: 'numeric' }; return new Intl.DateTimeFormat('tr-TR-u-ca-islamic-umalqura', options).format(date); } catch (e) { return "Hicri takvim alınamadı."; }};
...
export default function Home() {
    ...
    const [currentVakitDetail, setCurrentVakitDetail] = useState<any>(null);
    const [allVakitlerInternal, setAllVakitlerInternal] = useState<any[]>([]);
    const [currentDateTime, setCurrentDateTime] = useState(new Date());
    const [user, setUser] = useState<User | null>(null);
    const [vakitDefinitions, setVakitDefinitions] = useState<any[]>([]);
    ...
    const [nextVakitDetail, setNextVakitDetail] = useState<any>(null);
            ...
            if (currentUser) {
                ...
                const unsubSnapshot = onSnapshot(userSettingsRef, (docSnap) => {
                    if (docSnap.exists()) {
                        ...
                        const defaultSettings = { gunesDogus: '06:20', ogleVakti: '12:55', gunesBatis: '19:10', alarms: {} };
    ...
    useEffect(() => {
        ...
        let currentVakit = sortedVakitler.find(v => now >= new Date(v.start) && now < new Date(v.end));
        const progress = currentVakit ? (now.getTime() - new Date(currentVakit.start).getTime()) / (new Date(currentVakit.end).getTime() - new Date(currentVakit.start).getTime()) * 100 : 0;
        ...
        if (currentVakit) {
            ...
            let nextDefined = sortedVakitler.find(v => new Date(v.start) > now) || sortedVakitler[0];
            ...
            let prevDefined = sortedVakitler[prevDefinedIndex];
            setCurrentVakitDetail({ name: "Boş Vakit", activity: "Tanımlı vakit bekleniyor...", progress: 100, id: 'bos_vakit', start: prevDefined?.end, end: nextDefined?.start});
        }

        let nextVakit = sortedVakitler.find(v => new Date(v.start) > now);
        if(!nextVakit && sortedVakitler.length > 0) {
            let nextDayStart = new Date(sortedVakitler[0].start);
            nextDayStart.setDate(nextDayStart.getDate() + 1);
            nextVakit = {...sortedVakitler[0], start: nextDayStart.toISOString()};
        }
        setNextVakitDetail(nextVakit);
        ...
        <main className="flex min-h-screen flex-col items-center bg-gray-900 text-white p-4 font-sans">
            <div className="text-center w-full max-w-2xl mt-8">
                <div className="bg-gray-800 p-4 rounded-2xl shadow-lg">
                    ...
                    <p className="text-lg text-gray-400 my-2">{formatTime(currentVakitDetail?.start ? new Date(currentVakitDetail.start) : null)} - {formatTime(currentVakitDetail?.end ? new Date(currentVakitDetail.end) : null)}</p>
                    <div className="relative w-full my-3">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center text-xs font-bold text-white" style={{ textShadow: '1px 1px 2px rgba(0,0,0,0.7)' }}>
                            {currentVakitDetail ? `${Math.round(currentVakitDetail.progress)}%` : '0%'}
                        </div>
                    </div>
                                ...
                                <div className="p-3 flex items-center justify-between cursor-pointer" onClick={() => setExpandedVakitId(prevId => (prevId === vakit.id ? null : vakit.id))}>
                                    <div className="flex items-center truncate flex-grow">
                                        ...
                                        <div className="flex flex-col truncate">
                                            ...
                                            <span className="text-xs text-gray-400">{formatTime(new Date(vakit.start))} - {formatTime(new Date(vakit.end))} ({formatDuration(vakit.duration)})</span>
                                        </div>
                                    </div>
